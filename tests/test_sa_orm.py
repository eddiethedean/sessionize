...
class TestPrimaryKeys(unittest.TestCase):
    def primary_keys(self, setup_function):
...
class TestHasPrimaryKey(unittest.TestCase):
    def has_primary_key(self, setup_function):
        engine, table = setup_function()
...
class TestGetTable(unittest.TestCase):
    def get_table(self, setup_function):
        engine, table = setup_function()
        result = get_table('people', engine)
        expected = table
...
class TestGetClass(unittest.TestCase):
    def get_class(self, setup_function):
        engine, table = setup_function()
        result = get_class('people', engine)
...
class TestGetColumn(unittest.TestCase):
    def get_column(self, setup_function):
        engine, table = setup_function()
...
class TestSelectRecords(unittest.TestCase):
    def select_records(self, setup_function):
        engine, table = setup_function()
        expected = [
